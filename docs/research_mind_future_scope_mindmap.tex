...
\begin{center}
  ...
  \vspace{1mm}
  {\small\color{Muted}Simple roadmap: pehle trust improve karo, phir features aur scale add karo}
\end{center}
...
\begin{tikzpicture}[
  ...
  branch/.style={rounded corners=3mm,minimum height=25mm,text width=54mm,
  ...
  \node[branch,fill=GreenSoft,draw=Brand] (quality) at (-7.7,3.35) {
    ...
    $\bullet$ Writer--Critic revision loop\\
    $\bullet$ Citation-ID validation\\
    $\bullet$ Claim-to-source verification
  };

  \node[branch,fill=BlueSoft,draw=Blue] (knowledge) at (0,3.65) {
    \textbf{2. Deeper Research}\\[1mm]
    $\bullet$ Full-text/open-access papers\\
    $\bullet$ User PDF upload\\
    $\bullet$ Vector DB for saved knowledge
  };

  \node[branch,fill=OrangeSoft,draw=Orange] (search) at (7.7,3.35) {
    \textbf{3. Smarter Search}\\[1mm]
    $\bullet$ Multiple focused sub-queries\\
    $\bullet$ Relevance reranking\\
    $\bullet$ Conflict and evidence-gap search
  };

  \node[branch,fill=PurpleSoft,draw=Purple] (product) at (-7.7,-3.35) {
    \textbf{4. User Features}\\[1mm]
    $\bullet$ Research history and saved reports\\
    $\bullet$ Notes, bookmarks and comparison\\
    $\bullet$ Team sharing and comments
  };

  \node[branch,fill=GoldSoft,draw=Gold] (scale) at (0,-3.65) {
    \textbf{5. Production Scale}\\[1mm]
    $\bullet$ Background research jobs\\
    $\bullet$ Cache + database persistence\\
    $\bullet$ Auth, quotas and rate limits
  };

  \node[branch,fill=RedSoft,draw=Red] (safety) at (7.7,-3.35) {
    \textbf{6. Safety and Review}\\[1mm]
    $\bullet$ Source-quality scoring\\
    $\bullet$ Specialized critics\\
    $\bullet$ Human approval for high-stakes use
  };
...
\color{BrandDark}
Citation validation $\rightarrow$ Writer--Critic revision $\rightarrow$
Claim verification $\rightarrow$ PDF/full-text support $\rightarrow$ User history $\rightarrow$ Scale
\end{tcolorbox}
