...
Fourth, the \textbf{Critic Chain} checks the completed report. It gives a score, explains the strengths, and points out unsupported claims, bias, missing evidence, or overconfidence.

Gemini is the main model for the Search, Reader, and Writer stages. Groq is used as an optional independent Critic. If Groq is not configured, Gemini also performs the review. I kept this rule fixed instead of creating a complicated model-routing system.
...
One important limitation is that the system mainly uses abstracts and snippets. A valid source ID also does not guarantee that the source fully supports every claim. For high-stakes research, the user should open and verify the original sources. In the future, I would add automatic claim-citation verification and full-text support through official APIs.
