...
\begin{answerbox}
``I started with a tutorial-based Search, Reader, Writer, and Critic workflow, then upgraded it with live academic and news sources, source IDs, a responsive UI, and deployment support. Gemini handles the first three stages and Groq provides an optional independent Critic.''
\end{answerbox}

\subsection{Hinglish elevator pitch}

\begin{answerbox}
``Maine tutorial ke Search Agent, Reader Agent, Writer Chain aur Critic Chain ko base rakha. Search live papers/news laata hai, Reader evidence compare karta hai, Writer cited report banata hai aur Groq Critic weak claims check karta hai. Code ko jaan-boojhkar direct aur interview-friendly rakha hai.''
\end{answerbox}
...
\begin{enumerate}
  ...
  \item \textbf{Independent review:} Groq available ho to completed report ko separately check karta hai.
  \item \textbf{Simple model rule:} Gemini research, Groq critic; no routing matrix.
...
Yeh learning demo achha tha, lekin real product ke liye limitations thi: Tavily dependency, single scraped page, generic report, no strong evidence library, no source IDs, aur demo-like UI.
...
4 & \file{pipeline.py} & Original four sequential stages, simple dictionary state and progress callback banaye. \\
5 & \file{app.py} & Responsive Streamlit experience, progress, tabs, evidence cards aur follow-up chat banaya. \\
6 & Reliability & Query normalization, source timeouts, bounded snippets and deduplication add hue. \\
7 & Models & Simple rule rakha: Gemini research aur optional Groq critic. \\
...
\begin{itemize}
  ...
  \item Simple, explainable model selection.
...
Stages ka order tutorial jaisa dependency-driven hai. Reader ko Search results chahiye, Writer ko Reader notes chahiye, aur Critic ko completed report chahiye. Simplicity ke liye source calls bhi seedhe one-by-one run hote hain.

\interview{I kept the pipeline deliberately sequential so its execution order is obvious in an interview and easy to debug.}
...
Is file ko original project ki tarah direct rakha gaya hai:

\begin{enumerate}
  \item Environment variables load karna.
  \item Gemini main model aur optional Groq critic banana.
  \item Four prompt templates define karna.
  \item Har prompt ko \code{prompt | model | StrOutputParser()} chain mein badalna.
  \item Evidence-bound follow-up answer banana.
\end{enumerate}

\subsubsection{Simple model setup}

Project mein koi provider routing matrix nahi hai. Gemini Search, Reader aur Writer chalata hai. Groq key ho to Groq Critic chalata hai; key na ho to Gemini Critic bhi chala deta hai. Yeh ek hi simple fallback hai.

\begin{lstlisting}[language=Python,caption={Actual model selection idea}]
...
Why optional second provider? Independent Critic useful hai, but application ko samajhne ke liye complex router zaroori nahi. Interview answer seedha hai: ``Gemini research karta hai; Groq report review karta hai.''
...
\subsubsection{Straightforward collection}

Search functions ek simple loop mein run hote hain. Har selected source ka function call visible hai, isliye beginner easily debug kar sakta hai.

\begin{itemize}
  \item Ek source fail ho to baaki evidence retain hota hai.
  \item Error short warning mein convert hota hai.
  \item Trade-off: parallel version se thodi zyada latency ho sakti hai, lekin code ka learning cost kam hai.
\end{itemize}
...
\subsubsection{Simple function arguments}

Dataclass aur request adapters remove kar diye gaye. Main function seedhe topic aur optional form values leta hai:
\begin{center}
\code{run\_research\_pipeline(topic)}
\end{center}
Time range, domain, language aur source toggles normal keyword arguments hain. Yeh original video ke call ke sabse close hai.
...
\begin{enumerate}
  ...
  \item UI values direct pipeline function arguments bante hain.
  ...
  \item Search stage selected source functions one-by-one call karti hai.
...
\begin{tabular}{lcc}
\toprule
Depth & Per selected source limit & Research model \\
\midrule
Quick scan & 4 & Gemini \\
Standard & 6 & Gemini \\
Deep dive & 8 & Gemini \\
...
\begin{itemize}
  ...
  \item Direct string slices before Reader, Writer and Critic calls.
\end{itemize}

\textbf{Verification:} Standard pipeline live sources aur all four tutorial-aligned stages ke saath successfully run hui.

\begin{answerbox}
\textbf{STAR version:} Situation -- production-like run Critic stage par 413 se fail hua. Task -- request ko provider limit ke andar lana. Action -- source count, excerpt size, prompt input and output size directly cap ki. Result -- same four-stage flow smaller predictable context ke saath run hui.
\end{answerbox}
...
Hardcoded model IDs time ke saath retire ho sakte hain. Isliye model names environment variables mein configurable rakhe gaye. Interview mein exact model se zyada important point hai ki model name source code edit kiye bina change ho sakta hai.
...
Live OpenAlex/news test & External collectors returned structured source records. \\
...
\begin{enumerate}
  ...
  \item Gemini/Groq model creation tests.
...
Optional Groq review & More independent critique when configured & Second key is optional. \\
...
Sequential source calls & Easy to read and debug & Slower than parallel I/O. \\
Markdown output & Easy render/export & Structured downstream analytics harder than strict JSON. \\
Direct string limits & Simple context-size guard & Character count is not exact token count. \\
...
\begin{itemize}
  \item Source wall time is approximately \(O(T_1+\cdots+T_S)\) because calls are sequential.
...
\begin{answerbox}
A normal dictionary stores request data and each stage's output. Every stage reads the previous key and adds its own key.
\end{answerbox}

\textbf{Q5. Why not run all agents in parallel?}
\begin{answerbox}
Each stage needs the previous result. I also kept source calls sequential because this is a learning-focused project; the small latency trade-off makes the code much easier to explain and debug.
\end{answerbox}
...
\begin{answerbox}
Each source call has a small try/except. A failed API becomes a warning and the next selected source still runs.
\end{answerbox}
...
\begin{answerbox}
Gemini handles Search, Reader and Writer. Groq handles the Critic so the report can be reviewed by a different model. This is a fixed, easy-to-explain rule rather than automatic routing.
\end{answerbox}

\textbf{Q14. What if one API key is absent?}
\begin{answerbox}
Google key is required because Gemini runs the research stages. Groq is optional; without it Gemini also runs the Critic. There are no additional provider fallbacks.
\end{answerbox}
...
\begin{answerbox}
Bounded record counts, 500-character excerpts, direct string slices before each model call, and a maximum output-token setting.
\end{answerbox}

\subsection{Python and backend}

\textbf{Q19. Why normal function arguments instead of a request class?}
\begin{answerbox}
The project is small and interview-focused. Explicit function arguments keep the main pipeline call close to the tutorial and avoid introducing a class that adds little value here.
\end{answerbox}

\textbf{Q20. Why are source calls sequential?}
\begin{answerbox}
The application calls only two or three sources. Sequential requests make control flow obvious. If latency becomes important, parallel I/O is a measured future optimization.
\end{answerbox}

\textbf{Q21. Why did you remove automatic provider routing and client caching?}
\begin{answerbox}
They made a small learning project harder to explain. A fixed Gemini-research/Groq-critic rule is enough. I would add caching or routing only after profiling a real production workload.
\end{answerbox}
...
\begin{enumerate}
  ...
  \item \textbf{Models (15 sec):} simple rule explain karo---Gemini research, Groq review.
...
\begin{itemize}
  ...
  \item Upgraded a tutorial-based four-stage Search--Reader--Writer--Critic workflow with live evidence, source IDs, and optional cross-provider review.
  \item Used Gemini for research and an optional Groq critic, keeping model selection explicit and easy to maintain.
  \item Improved source reliability with timeouts, simple per-source error handling, deduplication, and stable source IDs.
...
\textbf{Action:} Measured stage context, bounded excerpts, capped outputs, used direct per-stage slices, and reduced source limits.\\
...
Question \(\rightarrow\) Search plan + live sources \(\rightarrow\) Reader notes \(\rightarrow\) Writer report \(\rightarrow\) Critic feedback \(\rightarrow\) Evidence-bound Q\&A.
\end{keybox}
...
Adversarial review & Optional Groq Critic checks the completed report. \\
Execution & Stages and source calls are sequential for readability. \\
Token budget & Bounded source count + excerpts + direct slices + output cap. \\
Fallback & Only Groq is optional; Gemini is required and can also run the Critic. \\
...
\begin{enumerate}
  ...
  \item ``I kept the flow sequential so every stage is easy to explain and debug.''
...
\begin{itemize}
  ...
  \item[$\square$] Why four stages, why Gemini + optional Groq, and why sequential flow explain kar sakte ho.
